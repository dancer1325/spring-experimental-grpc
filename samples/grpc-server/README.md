...
* 👀copy from [gRPC Spring Boot Starter](https://github.com/yidongnan/grpc-spring-boot-starter/blob/master/examples/local-grpc-server/build.gradle) 👀
...
* requirements
  * JVM is GraalVM
* steps
  * `$ ./mvnw -Pnative native:compile` or `$ ./gradlew -Pnative native:compile` 
    * compile to a native image 
  * `$ ./target/demo`
    * executed it
    ```
      .   ____          _            __ _ _
